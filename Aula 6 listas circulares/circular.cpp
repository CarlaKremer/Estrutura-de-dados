...
/*1*/ void inserirnaLista(int valor)
{
    ...
    if (prim == NULL)
        prim = p; // (p � auxiliar) inicio recebe aux

    p->prox = prim; //aux->prox recebe inicio (fora do if)

    if (q != NULL)   //se a não for for nula
...
/*2*/ void removerALista()
{
    ...
    if (achou == 0)
        printf("\nA lista n�o possui o elemente digitado\n");
    else
...
/*3*/ void exibirLista()
{
    ...
    } while (p != prim); //enquanto aux for diferente do inicio/ antes de voltar pro come�o
...
main()
{
    ...
    do
    {
        ...
        case 3:
            exibirLista();
            break;
        case 0: //para n�o exibir o "op��o inv�lida", j� que o 0 finaliza o programa
            break;
        default:
            printf("\nOp��o invalida!\n");
            getch();
